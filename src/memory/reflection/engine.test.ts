...
describe('ReflectionEngine', () => {
  ...
  test('应该拒绝非法 JSON 输出', async () => {
    ...
    mockAiModule(async () => ({ text: '这不是 JSON，是 LLM 胡言乱语' }));
  ...
  test('空洞察输出应该返回 executed=true 但无产出', async () => {
    ...
    mockAiModule(async () => ({ text: JSON.stringify({ insights: [], summary: '本次无洞察' }) }));
