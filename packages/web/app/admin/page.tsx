...
export default function AdminPage(): React.ReactElement {
  ...
  if (isAdmin === false) {
    return (
      <div className="spacing-lg max-w-6xl mx-auto">
        <PageHeader kicker="Administratio" title="管理面板" />
  ...
  return (
    <div className="spacing-lg max-w-6xl mx-auto">
      ...
      {/* Tab 导航(图鉴目录式下划线标记) */}
      <TabsRoot value={tab} onValueChange={(v) => setTab(v as "users" | "usage")}>
        <TabsList>
                ...
                {users.map((u) => (
                  <tr key={u.tenantId} className="border-b border-[var(--c-engraving-fine)]/40">
                    ...
                    <td className="py-3">
                      {u.petId ? (
                        <Button
                          variant={u.petStatus === "active" ? "danger" : "ghost"}
                          size="sm"
                          onClick={() =>
                            void setPetStatus(
                              u.tenantId,
                              u.petStatus === "active" ? "paused" : "active",
                            )
                          }
                          className={
                            u.petStatus === "active"
                              ? ""
                              : "text-[var(--c-amber-ink)] border border-[var(--c-engraving-fine)] hover:border-[var(--c-amber)]"
                          }
                        >
