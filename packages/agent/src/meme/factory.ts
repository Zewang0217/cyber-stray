 ...
 *
 * - 生图：qwen-image（DASHSCOPE_API_KEY；模型/尺寸 env 可配）
 * - 叠加：meme-overlay.py（python3 + PIL，服务器端中文叠加）
 * - 质检：qwen-vl 语义（DASHSCOPE_API_KEY）+ 结构检查
 * - 配额：MEME_DAILY_LIMIT（env，默认 3 张/天；0 = 不限）
 */

import { createImageGenerator, createVisionQc } from './qwen.js';
...
export function createMemePipelineDeps(dataDir: string): MemePipelineDeps {
  ...
  const imageModel = process.env.MEME_IMAGE_MODEL ?? 'wanx2.1-t2i-turbo';
  const visionModel = process.env.MEME_VL_MODEL ?? 'qwen-vl-max';
  const size = process.env.MEME_IMAGE_SIZE ?? '1024*1024';
