 ...
 *
 * 覆盖 PLAN 01-02 Task 1 �?7 �?consolidator 断言 + 3 �?archiveFile 断言�?
 *   1) merge �?store.saveMemory（双�?INDEX.md + .index.json�?
 *   2) 软删除：merge 后原文件�?`.archive/knowledge/` 下、原目录下不存在
 *   3) 阈值读 config（lowImportanceThreshold 命中/不命中）
 *   4) 过期：cleanupExpired 基于 indexed accessedAt 命中/不命�?
 *   5) merge store 缺失抛错（D-09�?
 *   6) D-04 双记：cleanup 后存�?observation + tags �?consolidation
 *   7) .archive 不被重扫（getRecentMemories 不含归档条目�?
 *   archive a) rename �?.archive/<type>/   b) sourcePath 不存在抛�?
 *   archive c) toSafeFilename �?`../`
 */
...
describe('MemoryConsolidator', () => {
  ...
  beforeEach(() => {
    ...
    // 预建四类记忆子目录，测试可直�?writeFileSync 造数据（store.saveMemory 也会自动建，�?
  ...
  test('merge �?store.saveMemory：合并记忆进 INDEX.md / .index.json', async () => {
    ...
    // 用文件名直接造同话题（merge �?filename �?topic 匹配�?
    // store.saveMemory 生成�?id 含时间戳，话题匹配不保证；这里直接手写两份带话题名的文件
    ...
    // 断言：merged 记忆通过 store.saveMemory 写入（INDEX.md �?topic-merged 条目�?
    const index = await store.readIndex();
    expect(index.recentMemories.some((id) => id.includes('llmtopic-merged'))).toBe(true);
  });

  test('软删除：merge 后原文件�?.archive/knowledge/ 下，原目录不存在', async () => {
    const knowledgeDir = join(dir, 'knowledge');
  ...
  test('阈值读 config：lowImportanceThreshold=0.2 命中低价值清�?, async () => {
    // 造一�?importance=0.15 �?timestamp 8 天前的记�?
    ...
    // config.consolidation.lowImportanceThreshold 默认 0.2�?.15 < 0.2 �?age>7 �?命中
    const merged = await consolidator.consolidateOldMemories();
    expect(merged).toBeGreaterThanOrEqual(1);
    // 被软删除到归档（basename �?toSafeFilename 再补 .md�?
  ...
  test('过期：accessedAt 30 天前的记忆不�?cleanupExpired 清理', async () => {
  ...
  test('D-04 双记：cleanup 后存�?observation + tags �?consolidation', async () => {
    ...
    // 双记：store 中存在一�?type=observation + tags �?consolidation 的记�?
  ...
  test('.archive 不被重扫：getRecentMemories 结果不含归档条目', async () => {
    ...
    // 归档�?.archive/ 存在文件，但 getRecentMemories 不应返回归档内容
    const recent = await store.getRecentMemories({ count: 50 });
...
describe('archiveFile', () => {
  ...
  test('rename 源文件到 .archive/<type>/ �?, async () => {
    const knowledgeDir = join(dir, 'knowledge');
    ...
    // WR-01：先�?.md 再净化后补回 �?归档名保持原 id（不再是 xxx-md.md�?
  ...
  test('toSafeFilename �?`../`：恶意文件名被净�?, async () => {
...
describe('loadBehaviorConfig 嵌套合并', () => {
  ...
  // W2 数据安全：部�?consolidation 配置时其它字段仍取默认值（�?undefined 阈值致误归档）
  test('部分 consolidation 配置时其它字段仍取默认值（W2 数据安全�?, async () => {
    // 构造只�?consolidation.expiryDays=10 �?agent-config.json（其余字段缺失）
