...
describe('MemoryStore', () => {
  ...
  test('saveMemory→getMemory 往返保持内容一�?, async () => {
    ...
    expect(got!.summary).toBe('DeepSeek V4 开�?);
    // content 原文被保留；注：parseMemoryFrontmatter 当前会在 content 前保�?## summary �?
    // （既有解析行为），故�?toContain 验证原文而非精确相等
    expect(got!.content).toContain('DeepSeek V4 开源，Agent 能力第一');
    expect(got!.tags).toEqual(['ai', 'llm']);
    expect(got!.importance).toBe(0.6);
  });

  test('importance>0.7 的记忆进�?importantMemories', async () => {
    const saved = await store.saveMemory({
      type: 'knowledge',
      timestamp: '2026-06-20T00:00:00.000Z',
      tags: ['重要'],
  ...
  test('saveMemory 累积 recentMemories �?tags（区段字段正确往返）', async () => {
  ...
  test('index 区段字段 write→read 往返一�?, async () => {
    ...
    // WR-06：概览区 key �?ASCII �?totalMemories / typeStats 也能正确往�?
 ...
 * 3) getRecentMemories 索引命中（不�?getMemory�?
 * 4) D-09 not found（getMemory→null / deleteMemory→false�?
 * 5) D-09 解析失败（非�?Markdown �?getMemory �?Error�?
 * 6) readIndex 解析失败（非�?INDEX.md �?readIndex �?Error�?
 * 7) deleteMemory 索引联动�?index.json records 不含该条�?
 */
describe('MemoryStore（索引层改�?/ MEM-01�?, () => {
  let dir: string;
  let store: MemoryStore;
  ...
  test('saveMemory 双写�?index.json records 含该条（�?INDEX.md 同步�?, async () => {
  ...
  // 2) getMemory 不读即写：文�?mtime 不变（写放大消除�?
  test('getMemory 不读即写：前后文�?mtime 不变', async () => {
    const saved = await store.saveMemory(makeEntryArgs());
    const filepath = join(dir, 'knowledge', `${toSafeFilename(saved.id)}.md`);

    // 两次 stat：读�?& 读后；时间需 <1s 间隔以保�?mtime 秒级粒度可靠
    const before = statSync(filepath).mtimeMs;
    await new Promise((r) => setTimeout(r, 1100)); // �?>1s（mtime 粒度�?
  ...
  // 3) getRecentMemories 索引命中：不�?getMemory（用 spy wrapper 计数�?
  test('getRecentMemories 索引命中：不�?getMemory（走 jsonIndex 直接 readFile�?, async () => {
    const saved = await store.saveMemory(makeEntryArgs());
  ...
  // 4) D-09 not found：getMemory→null / deleteMemory→false
  test('D-09 not found：getMemory �?null / deleteMemory �?false（合法空值）', async () => {
  ...
  // 5) D-09 解析失败：非�?Markdown �?getMemory �?Error（不�?null 兜底�?
  test('D-09 解析失败：非法内�?�?getMemory �?Error（不�?null�?, async () => {
    const saved = await store.saveMemory(makeEntryArgs());
    const filepath = join(dir, 'knowledge', `${toSafeFilename(saved.id)}.md`);
  ...
  // 6) readIndex 解析失败：非�?INDEX.md �?readIndex �?Error（不返默认）
  test('D-09 readIndex 解析失败：非�?INDEX.md �?�?Error（不返默认）', async () => {
    // 先写一条触�?INDEX.md 创建
    await store.saveMemory(makeEntryArgs());
    const indexPath = join(dir, 'INDEX.md');
    writeFileSync(indexPath, '完全非法的内�?###乱码', 'utf-8');
