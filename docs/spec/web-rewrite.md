...
10. **主题变体**：深夜霓虹（默认）/ 雨夜 / 白天（作息联动自动切，A 组 #3）/ 图鉴毛色皮肤（B 组，依赖毛色重映射能力）；手动切换器 = 顶栏下拉（demo 已示，#170 全局骨架 5）；主题色板切换走 tailwind v4 CSS 变量（DESIGN.md §7 一色一换）。
